...
import { createManyUrls } from './__test__/test-utils';
...
describe('UrlController (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    apiKey = configService.getOrThrow('apiKeys');
  ...
  describe('PATCH /url/:uid', () => {
    ...
    it('should return a 404 if the URL does not exist', () => {
      return request(server)
        .patch(`/url/invalid-random-uid`)
        .send({ title: 'Updated title' })
        .set('x-api-key', apiKey)
        .expect(404);
    });
